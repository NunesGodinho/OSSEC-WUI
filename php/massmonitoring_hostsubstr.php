...
if ($glb_debug == 1) {
    echo "var chartData3 = []";
    $hostsubstrdebugstring = "<div style='font-size:24px; color:red;'>Debug</div>";
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        //$hostsubstr = preg_replace("[^A-Za-z0-9]", "", $hostsubstr);
        $hostsubstr = str_replace("-", "", str_replace(")", "", str_replace("(", "", preg_replace("[^A-Za-z0-9]", "", $row['res_name']))));

        if (!isset($hostsubstrarray[$row['res_time']][$hostsubstr])) {
            // !array_key_exists($row['res_time'][$hostsubstr],$hostsubstrarray)
            $hostsubstrarray[$row['res_time']][$hostsubstr] = 0;
    ...
    foreach ($hostsubstrarray as $key => $val) {
        ...
        $mainstring3 = preg_replace('/,$/', '', $mainstring3);
        $mainstring3 .= "}";
    }
    $mainstring3 = preg_replace('/,$/', '', $mainstring3);
    $mainstring3 .= "
    ...
    foreach ($substrs2 as $substr) {
        $safe_substr = preg_replace("[^A-Za-z0-9]", "", $substr);
        $graphsubstr .= "
			// Graph " . $substr . "
	                // GRAPH
