...
while ($rowchart = $stmt->fetch()) {
    ...
    if ($tmpdate != $rowchart['res_time'] && $rowchart['res_time'] > 1) {
        ...
        $mainstring .= '		{"date": "' . date("Y", $tmpdate) . '-' . (date("m", $tmpdate) - 1) . '-' . date("j", $tmpdate) . ', ' . date("G", $tmpdate) . ':' . date("i", $tmpdate) . '",';
...
if ($tmpdate > 1) {
    ...
    $mainstring .= '},
			{"date": "' . date("Y", $timedown) . '-' . (date("m", $timedown) - 1) . '-' . date("j", $timedown) . ', ' . date("G", $timedown) . ':' . date("i", $timedown) . '", \'now\':1, ';
